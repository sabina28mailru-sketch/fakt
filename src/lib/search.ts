...
/**
 * Техническая ли это строка открытия страниц.
 *
 ...
 * результате, по ссылкам. Поэтому всё, кроме неудач, помечается техническим
 * и по умолчанию не показывается.
 *
 * Признак отдельный, а не свой вид строки: вид решает, каким цветом и в
 * какой счётчик. Стоило увести открытие страниц в отдельный вид — и счётчик
 * «Открыто» честно показал ноль, хотя страницы открывались.
 *
 * Неудача — другое дело: она объясняет, почему источников меньше ожидаемого.
 */
export function isTechLog(kind: "fetch" | "result" | "warn"): boolean {
  return kind !== "warn";
}
