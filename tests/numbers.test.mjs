...
test("текущий год не считается выдуманным числом", () => {
  // «В 2026 году алгоритмы работают так-то» — указание времени, а не данные.
  // На живом прогоне это дало семь ложных срабатываний и три лишних вызова.
  const now = new Date("2026-09-23T00:00:00Z");
  const frames = [{ where: "сторис, кадр 7", text: "В 2026 году правила поменялись" }];
  assert.deepEqual(checkNumbers(frames, ["текст без единой цифры"], now), []);
});

test("но чужой год проверяется как обычное число", () => {
  const now = new Date("2026-09-23T00:00:00Z");
  const frames = [{ where: "карусель, слайд 2", text: "Фильм вышел в 2012 году" }];
  const bad = checkNumbers(frames, ["на странице про это ни слова"], now);
  assert.equal(bad.length, 1);
  assert.equal(bad[0].value, "2012");
});
